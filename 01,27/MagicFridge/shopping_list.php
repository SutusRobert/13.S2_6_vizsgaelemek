...
/* ================================
   Segédek (raktár ellenőrzés + tipp helyre)
   ================================ */
...
function findInventoryByNameLoose(PDO $pdo, int $householdId, string $needle): ?array {
    ...
    foreach ($all as $r) {
        ...
        if ($n !== '' && (mb_strpos($needle, $n) !== false || mb_strpos($n, $needle) !== false)) {
            return $r;
        }
    }
...
/**
 * Measure parser (receptből jövő "measure" feldolgozás)
 */
...
function toStorePack(string $name, float $recipeQty, ?string $recipeUnit): array {
    ...
    if (str_contains($n, 'tejfol') || str_contains($n, 'tejföl') || str_contains($n, 'sour cream')) {
        return [1, 'doboz'];
    }
    ...
    $produceKeywords = ['hagyma','onion','fokhagyma','garlic','citrom','lemon','lime','alma','apple','paradicsom','tomato','uborka','cucumber','paprika','pepper','banan','narancs','orange'];
    foreach ($produceKeywords as $k) {
        if (str_contains($n, normalizeForMatch($k))) {
            if ($u === 'pcs' || $u === 'db') return [max(1, (int)ceil($recipeQty)), 'db'];
            return [1, 'db'];
        }
    }

    $spiceMarkers = ['tsp','tbsp','cup','pinch','to taste','csipet','ize'];
    foreach ($spiceMarkers as $m) {
        if (($u !== null && str_contains($u, $m)) || str_contains($n, normalizeForMatch($m))) {
            return [1, 'zacskó'];
        }
    }

    $spiceNames = ['spice','masala','paprika','bors','so','só','koriander','komeny','kömény','kurkuma','curry','oregano','bazsalikom','fahéj','cinnamon','gyomber','gyömbér'];
    foreach ($spiceNames as $k) {
        if (str_contains($n, normalizeForMatch($k))) {
            return [1, 'zacskó'];
        }
    }

    $canKeywords = ['konzerv','can','tin','passata','paradicsom','tomato','csicseriborso','csicseriborsó','bab','beans','kukorica','corn'];
    foreach ($canKeywords as $k) {
        if (str_contains($n, normalizeForMatch($k))) {
            return [1, 'doboz'];
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_missing_api') {
        ...
        if (is_array($missingItems)) {
            foreach ($missingItems as $it) {
                ...
                [$qty, $unit] = toStorePack($nm, (float)$rq, $ru);
    ...
    if ($action === 'add_missing_for_own_recipe') {
        ...
        $inv = findInventoryByNameLoose($pdo, $householdId, $ingredientName);

        if (!$inv) {
            $errors[] = "Nincs ilyen termék a raktárban: " . $ingredientName;
        } else {
            // itt jön a levonás az $inv['id'] alapján
        }
    ...
    if ($action === 'toggle') {
        ...
        if ($item) {
            ...
            try {
                ...
                // ha megvett, töltsük fel raktárba
                if ($to === 1) {
                    ...
                    // ugyanazt keressük (name + location + unit) hogy ne keveredjenek egységek
                    if ($unit !== null && trim($unit) !== '') {
                        $find = $pdo->prepare("
                            SELECT id
                            FROM inventory_items
                            WHERE household_id = ? AND name = ? AND location = ? AND ((unit IS NULL AND ? IS NULL) OR unit = ?)
                            ORDER BY id DESC
                            LIMIT 1
                        ");
