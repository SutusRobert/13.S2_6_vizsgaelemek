...
<head>
    ...
    <script>
        ...
        /* ------------------------------
           ...
           ------------------------------ */
        function createIngredientRow(value = '', qty = '') {
            const row = document.createElement('div');
            row.className = 'ing-row';
            row.style.display = 'grid';
            row.style.gridTemplateColumns = '1fr 140px 44px';
            row.style.gap = '10px';
        ...
        function addIngredient(value = '', qty = '') {
            const cont = document.getElementById('ingredients');
            const row = createIngredientRow(value, qty);
        ...
        function updatePreview(){
            ...
            const nameInputs = Array.from(document.querySelectorAll('#ingredients input[name="ingredients[]"]'));
            const qtyInputs  = Array.from(document.querySelectorAll('#ingredients input[name="quantities[]"]'));

            const rows = nameInputs.map((inp, i) => {
                const name = (inp.value || '').trim();
                const qty  = (qtyInputs[i]?.value || '').trim();
                return { name, qty };
            });

            const cleaned = rows.filter(r => r.name.length > 0);
            ...
            const empties = rows.length - cleaned.length;
            ...
            cleaned.slice(0, 12).forEach((r, idx) => {
                const row = document.createElement('div');
                row.className = 'preview-item';
                row.innerHTML =
                    '<div><b>' + escapeHtml(r.name) + '</b>' + (r.qty ? ('<br><small class="muted">' + escapeHtml(r.qty) + '</small>') : '') + '<br><small class="muted">#' + (idx+1) + '</small></div>' +
...
<div class="navbar">
    <div class="nav-left">
        <a href="dashboard.php" class="nav-brand-link" aria-label="Vissza a dashboardra" style="display:flex; align-items:center; gap:10px; text-decoration:none;">
            <img src="assets/Logo.png" class="nav-logo" alt="Logo">
            <span class="nav-title nav-title--static" style="text-decoration:none;">MagicFridge</span>
        </a>
    </div>
    <div class="nav-links">
