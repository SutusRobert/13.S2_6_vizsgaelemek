...
// párosítjuk soronként (ingredients[] + quantities[])
$ingredientsRows = [];
foreach ($ingredients as $i => $ing) {
    $name = trim((string)$ing);
    $qty  = trim((string)($quantities[$i] ?? ''));
    if ($name !== '') {
        $ingredientsRows[] = ['name' => $name, 'qty' => ($qty !== '' ? $qty : null)];
    }
}

$ingredientsClean = array_map(fn($r) => $r['name'], $ingredientsRows);
...
try {
    ...
    // ingredients
    $stIng = $pdo->prepare("INSERT INTO recipe_ingredients (recipe_id, ingredient, quantity) VALUES (?, ?, ?)");
    foreach ($ingredientsRows as $row) {
        $stIng->execute([$recipeId, $row['name'], $row['qty']]);
    }
