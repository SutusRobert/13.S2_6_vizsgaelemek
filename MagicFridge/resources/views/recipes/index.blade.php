...
<div class="main-wrapper">
  <div class="card" style="max-width: 980px; width:100%;">
    ...
    <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; align-items:flex-start;">
      <div>
        <h2 style="margin:0;">Receptek</h2>
        <div class="small" style="opacity:.8; margin-top:4px;">
          Háztartás: <b>{{ $activeHouseholdName ?? '' }}</b>
        </div>
      </div>

      <div style="display:flex; gap:10px; flex-wrap:wrap;">
        <a class="btn btn-secondary" href="{{ route('dashboard') }}">Vissza</a>
      </div>
    </div>
    ...
    <div class="mt-3" style="display:flex; gap:12px; flex-wrap:wrap; align-items:flex-end;">
      ...
      <div style="min-width:260px; flex:2;">
        <label class="small" style="opacity:.85;">Keresés (TheMealDB)</label>
      ...
      <div>
        <a class="btn btn-secondary" href="{{ route('recipes.index', ['hid'=> (int)($hid ?? 0)]) }}">Saját recept</a>
      </div>
    </div>
    ...
    {{-- Találatok (API) --}}
    <h3 style="margin:0;">Találatok (API)</h3>

    @php
      $apiMeals = $meals ?? $apiMeals ?? [];
      // normalizáljuk tömbre
    ...
    @else
      ...
      ">
        @foreach($apiMeals as $m)
          @php
            // TheMealDB mezők: idMeal, strMeal, strMealThumb
            $idMeal = (int)($m['idMeal'] ?? $m->idMeal ?? 0);
            $nameEn = (string)($m['strMeal'] ?? $m->strMeal ?? 'Recept');
            $thumb  = (string)($m['strMealThumb'] ?? $m->strMealThumb ?? '');

            // ha a controller már küldi magyar nevet, akkor használjuk:
            $nameHu = (string)($m['name_hu'] ?? $m->name_hu ?? '');
